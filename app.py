...
.custom-footer {
  ...
  border-top: 2px solid #2c3e50; margin-top: 10mm; padding-top: 4mm;
...
with tab1:
    ...
    with left_col:
        ...
        for i in range(int(n_supp)):
            ...
            with c2:
                kind = st.selectbox(
                    ...
                    index=["pin","roller","fixed","hinge"].index(default_kind if default_kind in ["pin","roller","fixed","hinge"] else "pin"),
                    key=f"supp_kind_{i}"
                )
        ...
        if use_conc_hydro:
            ...
            a_eff = max(x_start, x_end - Lh); b_eff = x_end
            a_clip = max(0.0, min(a_eff, L)); b_clip = max(0.0, min(b_eff, L))
        ...
        # Solve button (kept outside any form => live rerun)
